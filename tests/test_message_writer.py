...
@mock.patch(
    "rb_quality_plugin.core.message_writers.bigquery_writer.BigQueryHook", autospec=True
)
@mock.patch(
    "rb_quality_plugin.core.message_writers.bigquery_writer.Client", autospec=True
)
def test_bigquery_writer(mock_client, mock_hook):
    ...
    writer = BigQueryWriter(
        connection_id="google_cloud_default", table_id=test_table_id
    )
    writer.send_message(test_message)

    mock_client().insert_rows.assert_called_once_with(
        test_table_id, [tuple(i[1] for i in test_message.items())]
    )
...
def test_multiwriter(mock_send_message):
    ...
    writer = MultiWriter(
        writers=[
            MessageWriter(connection_id="test_conn1"),
