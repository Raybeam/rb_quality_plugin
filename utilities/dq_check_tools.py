...
def create_dq_checks_from_list(dag, dq_check_list):
    ...
    :param dq_check_list: list of tuples each containing a dq config path & dq check arguments
...
def create_dq_checks_from_directory(dag, dq_check_dir, dq_check_args={}, rec=True):
    ...
    :param rec: (optional) recursively search through sub-directories (default True)
    :type rec: bool
